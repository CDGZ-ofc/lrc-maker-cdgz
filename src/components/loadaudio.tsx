        ...
        <details ref={loadAudioDialogRef} className="dialog fixed loadaudio-dialog" onToggle={onToggle}>
            ...
            <section className="dialog-body loadaudio-body">
                <div className="loadaudio-content">
                    <label className="audio-input-tip ripple" htmlFor="audio-input">
                ...
                <div className="loadaudio-content">
                    <form className="audio-input-form" onSubmit={onSubmit}>
                        <input
                            ...
                            spellCheck={false}
                            onFocus={onFocus}
                            placeholder={lang.loadAudio.url}
                        />
...
export const nec = (url: string): string => {
    // Check if URL is a pure number (网易云音乐ID)
    const isPureNumber = /^\d+$/.test(url);
    if (isPureNumber) {
        return `https://music.163.com/song/media/outer/url?id=${url}.mp3`;
    }

    // Check if URL contains music.163.com (网易云音乐链接)
