...
export const Content: React.FC = () => {
    ...
    // Removed automatic [length] tag generation
    ...
    // Ref to track if we've already processed the text parameter (use ref instead of state to avoid re-renders)
    const isTextParamProcessed = useRef(false);

    // Load lyric text from URL query parameter
    useEffect(() => {
        const getLyricFromUrl = async () => {
            // Check if we've already processed this text parameter
            if (isTextParamProcessed.current) {
                return;
            }

            // Check location.search first (e.g., /?text=https://example.com/lyric.txt)
            let textUrl: string | null = null;

            if (location.search && URLSearchParams) {
                const searchParams = new URLSearchParams(location.search);
                textUrl = searchParams.get("text");
            }

            // Then check location.hash for query params (e.g., /#/?text=https://example.com/lyric.txt)
            if (!textUrl && location.hash && URLSearchParams) {
                const hash = location.hash;
                const hashQueryIndex = hash.indexOf("?");
                if (hashQueryIndex !== -1) {
                    const hashQuery = hash.slice(hashQueryIndex);
                    const hashSearchParams = new URLSearchParams(hashQuery);
                    textUrl = hashSearchParams.get("text");
                }
            }

            if (textUrl) {
                try {
                    // Mark as processed before fetching to prevent duplicate calls
                    isTextParamProcessed.current = true;

                    // Fetch the lyric text from the URL
                    const response = await fetch(textUrl);
                    const text = await response.text();

                    // Ask user for confirmation before importing lyric text
                    const confirmImport = confirm(
                        "是否确认导入text参数？此操作将覆盖当前歌词。\nAre you sure you want to import this lyric text?\nThis will overwrite the current lyric.",
                    );

                    if (confirmImport) {
                        // Parse and update the lyric state
                        lrcDispatch({
                            type: LrcActionType.parse,
                        ...
                        if (path.slice(1) !== ROUTER.editor) {
                            location.hash = ROUTER.editor;
                        }
                    }
                } catch (error) {
                    console.error("Failed to load lyric text from URL:", error);
                    // Keep as processed even on error to avoid repeated attempts
                }
            }
        };

        getLyricFromUrl();
    }, [location.search, location.hash, lrcDispatch, path, trimOptions]);
