...
export const Synchronizer: React.FC<ISynchronizerProps> = ({ state, dispatch }) => {
        ...
        function onKeydown(ev: KeyboardEvent): void {
            ...
            switch (action) {
                ...
                case InputAction.DecreaseOffset:
                    ev.preventDefault();
                    adjust(ev, -prefState.fineTuneOffset / 1000, selectIndex);
                    break;
                ...
                case InputAction.PrevLine:
                    ev.preventDefault();
                    {
                        const newIndex = Math.max(0, selectIndex - 1);
                        dispatch({ type: ActionType.select, payload: () => newIndex });
                        // Efficiency mode: jump to timestamp when using keyboard to select line
                        if (prefState.efficiencyMode) {
                            const selectedLine = lyric[newIndex];
                            if (selectedLine?.time !== undefined && selectedLine.time > 0) {
                                audioRef.currentTime = selectedLine.time;
                            }
                        }
                    }
                    break;
                case InputAction.NextLine:
                    ev.preventDefault();
                    {
                        const newIndex = Math.min(lyric.length - 1, selectIndex + 1);
                        dispatch({ type: ActionType.select, payload: () => newIndex });
                        // Efficiency mode: jump to timestamp when using keyboard to select line
                        if (prefState.efficiencyMode) {
                            const selectedLine = lyric[newIndex];
    ...
    }, [adjust, dispatch, keyBindings, lyric, selectIndex, sync, prefState.efficiencyMode]);
    ...
    const onLineClick = useCallback(
        (ev: React.MouseEvent<HTMLUListElement & HTMLLIElement>) => {
            ...
            if (target.classList.contains("line")) {
                ...
                // Efficiency mode: jump to timestamp when clicking line
                if (prefState.efficiencyMode) {
                    const selectedLine = lyric[lineKey];
                    if (selectedLine?.time !== undefined && selectedLine.time > 0) {
                        audioRef.currentTime = selectedLine.time;
                    }
                }
        ...
        [dispatch, lyric, prefState.efficiencyMode],
    );
