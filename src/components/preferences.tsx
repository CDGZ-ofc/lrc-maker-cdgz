...
export const Preferences: React.FC = () => {
    ...
    const onStepSecondsChange = useCallback(
        (ev: React.ChangeEvent<HTMLInputElement>) => {
            prefDispatch({
                type: "stepSeconds",
    ...
    const onFineTuneOffsetChange = useCallback(
        (ev: React.ChangeEvent<HTMLInputElement>) => {
            prefDispatch({
                type: "fineTuneOffset",
                payload: Number(ev.target.value),
    ...
    const onEfficiencyModeToggle = useCallback(
        () =>
            prefDispatch({
                type: "efficiencyMode",
                payload: (prefState) => !prefState.efficiencyMode,
            }),
        [prefDispatch],
    );
    ...
    const onAllowBackgroundAudioToggle = useCallback(
        () =>
            prefDispatch({
                type: "allowBackgroundAudio",
                payload: (prefState) => !prefState.allowBackgroundAudio,
            }),
        [prefDispatch],
    );
    ...
    const formatedText = useMemo(() => {
        return formatText("   Guangzhou   广州～   ", prefState.spaceStart, prefState.spaceEnd);
    ...
    return (
        <div className="preferences">
            <ul>
                ...
                <li>
                    <label className="list-item">
                        <span>{lang.preferences.efficiencyMode}</span>
                        <label className="toggle-switch">
                            <input
                                type="checkbox"
                                checked={prefState.efficiencyMode}
                                onChange={onEfficiencyModeToggle}
                                aria-label={lang.preferences.efficiencyMode}
                            />
                            <span className="toggle-switch-label" />
                        </label>
                    </label>
                ...
                <li>
                    <label className="list-item">
                        <label htmlFor="step-seconds">{lang.preferences.stepSeconds}</label>
                        <input
                            name="stepSeconds"
                            id="step-seconds"
                            required={true}
                            min={100}
                            max={30000}
                            {...useNumberInput(prefState.stepSeconds, onStepSecondsChange)}
                        />
                    </label>
                </li>
                <li>
                    <label className="list-item">
                        <label htmlFor="fine-tune-offset">{lang.preferences.fineTuneOffset}</label>
                        <input
                            name="fineTuneOffset"
                            id="fine-tune-offset"
                            required={true}
                            min={10}
                            max={5000}
                            {...useNumberInput(prefState.fineTuneOffset, onFineTuneOffsetChange)}
                        />
                    </label>
                </li>
