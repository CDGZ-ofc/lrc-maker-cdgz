...
const offsetInput = timeTransform.elements.namedItem("offset") as HTMLInputElement;
const offsetDefaultSetBtn = $(".offset-default-set") as HTMLButtonElement;
const offsetDefaultApplyBtn = $(".offset-default-apply") as HTMLButtonElement;
...
// Load default offset from localStorage
const DEFAULT_OFFSET_KEY = "lrc-maker-default-offset";
const loadDefaultOffset = () => {
    const savedOffset = localStorage.getItem(DEFAULT_OFFSET_KEY);
    if (savedOffset) {
        offsetDefaultApplyBtn.disabled = false;
    }
};

// Set current offset as default
const setDefaultOffset = () => {
    const currentOffset = offsetInput.value;
    localStorage.setItem(DEFAULT_OFFSET_KEY, currentOffset);
    offsetDefaultApplyBtn.disabled = false;
    alert(`已设置为默认偏移值：${currentOffset}ms`);
};

// Apply default offset
const applyDefaultOffset = () => {
    const savedOffset = localStorage.getItem(DEFAULT_OFFSET_KEY);
    if (savedOffset) {
        offsetInput.value = savedOffset;
        // Manually update the output
        const scale = {
            a: 1,
            c: Number.parseInt(savedOffset) / -1000,
        };
...
const main = () => {
    ...
    // Load default offset and add event listeners
    loadDefaultOffset();
    offsetDefaultSetBtn.addEventListener("click", setDefaultOffset);
    offsetDefaultApplyBtn.addEventListener("click", applyDefaultOffset);
};
